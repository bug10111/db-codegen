...
/// <summary>
/// AI 模板生成向导视图模型，承载向导闭环：LLM 配置检查、技术栈描述与样例表选择、
/// 调用 AI 模板生成服务、结果展示（成功跳转模板包管理 / 失败错误清单+重试+保留原文）。
/// 样例表候选集取自主窗口①区已加载表或经当前连接刷新，选中后惰性读取真实列元数据；
/// 用户包同名冲突在向导内弹覆盖确认后以覆盖标志重试，内置包同名只读拒绝直接失败。
/// 生成全程经取消令牌贯穿，向导关闭或停止按钮取消在途生成。
/// </summary>
public sealed partial class AiTemplateWizardViewModel : ObservableObject
{
    ...
    private readonly ILogger<AiTemplateWizardViewModel> _logger;
    ...
    /// <summary>
    /// 是否注入 easycode 参考素材作为转写参照，可选。
    /// </summary>
    [ObservableProperty]
    private bool _includeEasyCodeReference;
    ...
    /// 对话框服务、设置窗口工厂与模板包管理窗口工厂构造 AI 向导视图模型。
    /// </summary>
    /// <param name="configService">配置服务，读取 LLM 配置检查向导可用性。</param>
    ...
    /// <param name="logger">向导视图模型日志器，日志不输出 apiKey 与 LLM 原始输出。</param>
    /// <exception cref="ArgumentNullException">任一依赖参数为 null 时抛出。</exception>
    public AiTemplateWizardViewModel(
        ...
        ILogger<AiTemplateWizardViewModel> logger)
    {
    ...
    /// <summary>
    /// 向导窗口呈现后初始化：快照样例表候选集并执行 LLM 配置检查，未配置时提示跳转设置页。
    /// </summary>
    public async Task InitializeAsync()
    {
        // 从主窗口①表列表区快照当前已加载表作为样例表候选集，向导为模态窗口期间表区不变化
    ...
    /// <summary>
    /// 取消在途生成与样例表读取，供向导关闭或停止时调用，核心调用链经取消令牌感知取消。
    /// </summary>
    ...
    {
        // 生成前置校验：LLM 配置齐全且样例表真实元数据就绪，任一不满足不进入生成
